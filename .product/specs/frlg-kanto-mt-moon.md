...
Delivery: Ported to the `kanto-local-adventures` task for current Wayfarer.
The donor implementation in [PR #86](https://github.com/mzpkdev/pokemon-wayfarer/pull/86)
is source evidence. Current validation is reported with this task.
...
were checked in the donor E2E emulator ROM and by collision/elevation/route
audit. The current port retains the same cave layout and checks walkable access
and routes in its map test. Emulator inspection of the current port remains
part of integration validation. No tile, collision, elevation, or `map.bin`
change is made.
...
Each placed Rocket object uses the existing `OBJ_EVENT_GFX_ROCKET_M`. It preserves the
...
Use only these new Wayfarer-local flags:

| Flag | Name | Meaning |
| ---: | --- | --- |
| `0x954` | `FLAG_HIDE_MT_MOON_DOME_FOSSIL_HNS` | Hides the local Dome Fossil event. |
| `0x955` | `FLAG_HIDE_MT_MOON_HELIX_FOSSIL_HNS` | Hides the local Helix Fossil event. |
| `0x956` | `FLAG_GOT_FOSSIL_FROM_MT_MOON_HNS` | The player finalized this local choice. |
...
both events and sets `0x956` only after its selected-item transaction succeeds
...
or with setting the corresponding hide flags and `0x956`. The immediate
...
Add five Wayfarer-only trainers in the current shared trainer registry,
preserving every source roster, class, portrait, AI, music, and authored moves.
The current Wayfarer runtime IDs are 1768 through 1772.

| Runtime ID | New symbol | Source trainer | Exact normal roster | Scaling policy |
| --- | --- | --- | --- | --- |
| 1768 | `TRAINER_MT_MOON_ROCKET_GRUNT_1_HNS` | `TRAINER_TEAM_ROCKET_GRUNT` (251) | Rattata 13, Zubat 13 | `ORDINARY` |
| 1769 | `TRAINER_MT_MOON_ROCKET_GRUNT_2_HNS` | `TRAINER_TEAM_ROCKET_GRUNT_2` (252) | Sandshrew 11, Rattata 11, Zubat 11 | `ORDINARY` |
| 1770 | `TRAINER_MT_MOON_ROCKET_GRUNT_3_HNS` | `TRAINER_TEAM_ROCKET_GRUNT_3` (253) | Zubat 11, Ekans 11 | `ORDINARY` |
| 1771 | `TRAINER_MT_MOON_ROCKET_GRUNT_4_HNS` | `TRAINER_TEAM_ROCKET_GRUNT_4` (254) | Rattata 13, Sandshrew 13 | `ORDINARY` |
...
Static coverage must prove the seven event placements and their exact
