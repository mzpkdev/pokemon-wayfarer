...
Delivery: `task/kanto-local-adventures`, alongside the Mt. Moon, Safari, and Nugget Bridge local adventures. Validation evidence belongs to this delivery; the donor implementation is not a validation result for current main.
...
Steven keeps the existing HNS lobby interaction and Hoenn starter gift. The
gift and its receipt are independent of Silph occupation and liberation.
...
Define ITEM_SILPH_CARD_KEY as a new Wayfarer item after existing item IDs, and increase Wayfarer's item count accordingly. Keep all existing IDs and standalone
...
Allocate separate persistent Wayfarer-only flags for the following local states.
Keep their values disjoint from other story and daily flags, and define the
Silph symbols as zero in other products.

| Count | Meaning |
| ---: | --- |
| 1 | FLAG_SILPH_CARD_KEY_RECEIVED_HNS |
| 16 | Ordinary visible-item receipts |
| 12 | Hidden-item receipts |
| 20 | Persistent door-open states |
| 1 | FLAG_SILPH_LIBERATED_HNS |
| 1 | FLAG_GOT_SILPH_LAPRAS_HNS |
| 1 | FLAG_SILPH_THUNDER_WAVE_TUTOR_HNS |
| 1 | FLAG_SILPH_MASTER_BALL_REWARD_PENDING_HNS |
| 1 | FLAG_SILPH_MASTER_BALL_RECEIVED_HNS |
...
Append 31 Wayfarer Trainer records after the reserved Celadon Hideout trainer block. Their runtime IDs are 1737 through 1767.
...
| ---: | --- | --- |
| 1737 through 1755 | TRAINER_TEAM_ROCKET_GRUNT_23 through _41 | ORDINARY; hostile faction fallback through the ordinary resolver |
| 1756 through 1765 | Scientists Beau, Connor, Ed, Jerry, Jose, Joshua, Parker, Rodney, Taylor, and Travis | ORDINARY; stable NP_ORDINARY encounter profile |
| 1766 | TRAINER_JUGGLER_DALTON | ORDINARY; stable NP_ORDINARY encounter profile |
| 1767 | TRAINER_BOSS_GIOVANNI_2 | Objective-boss exclusion; NP_ROCKET_GUARD |
...
Steven's current lobby script remains reachable in Wayfarer. Its existing
starter choice, gift, and receipt are not gated by Silph state.
...
the allocated Silph Card Key ID, unchanged Goldenrod Card Key behavior, and full-pocket retries.
...
successful receipt, and continued 2F access. Confirm Steven's existing Hoenn
starter interaction remains available while unclaimed, regardless of whether
Silph has been liberated, with its existing receipt behavior. Validate the President reward
