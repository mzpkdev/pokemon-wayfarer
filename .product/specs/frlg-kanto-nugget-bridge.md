...
Delivery: `task/kanto-local-adventures`, adapted from the earlier donor milestone
to the current Wayfarer trainer and flag allocations.
...
The current HNS layout is 30 by 22. Its static tile data verifies
...
Allocate exactly one Wayfarer-local receipt flag:

| Flag | Name | Meaning |
| ---: | --- | --- |
| `0x95A` | `FLAG_NUGGET_BRIDGE_NUGGET_RECEIVED_HNS` | This episode's one `ITEM_NUGGET` was successfully granted. |

Define the flag as `0x95A` only in Wayfarer and zero in other products, using
...
| --- | --- | --- | --- |
| 1773 | `TRAINER_NUGGET_BRIDGE_CALE_HNS` | `TRAINER_BUG_CATCHER_CALE` | Caterpie 10, Weedle 10, Metapod 10, Kakuna 10 |
| 1774 | `TRAINER_NUGGET_BRIDGE_ALI_HNS` | `TRAINER_LASS_ALI` | Pidgey 12, Oddish 12, Bellsprout 12 |
| 1775 | `TRAINER_NUGGET_BRIDGE_TIMMY_HNS` | `TRAINER_YOUNGSTER_TIMMY` | Sandshrew 14, Ekans 14 |
| 1776 | `TRAINER_NUGGET_BRIDGE_RELI_HNS` | `TRAINER_LASS_RELI` | Nidoran M 16, Nidoran F 16 |
| 1777 | `TRAINER_NUGGET_BRIDGE_ETHAN_HNS` | `TRAINER_CAMPER_ETHAN` | Mankey 18 |
| 1778 | `TRAINER_NUGGET_BRIDGE_ROCKET_HNS` | `TRAINER_TEAM_ROCKET_GRUNT_6` | Ekans 15, Zubat 15 |
...
Wayfarer count to 1779. Add range assertions proving the six IDs are
contiguous and that the Rocket's runtime ID plus one equals
`TRAINERS_COUNT_WAYFARER`. The new records use current Wayfarer Trainer defeat
storage and do not enter Hoenn's fixed runtime bank.
...
output against the current-main baseline, and prove that standalone FRLG source
maps and scripts are unchanged.

Native tests and generator audits must cover runtime IDs 1773 through 1778,
total count 1779, exact source-party parity,
