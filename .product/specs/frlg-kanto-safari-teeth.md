...
Implementation: `task/kanto-local-adventures`. This local adventure is independent of the other Kanto adventures and of Celadon Hideout. Validation evidence belongs in the task note and delivery report.
...
Allocate three unused HNS flags in the current shared namespace. They
are Wayfarer-local state, with no reuse of zero-valued HNS aliases or HNS/Hoenn
progress flags:

| Flag | Meaning |
| --- | --- |
| `0x957` | The Beach Gold Teeth item ball has been claimed and is hidden. |
| `0x958` | The player completed the local Surf delivery at the Beach attendant. |
| `0x959` | The player completed the local Baoba Strength delivery. |
...
the item. It sets `0x957` and hides only after that successful grant. If the
player already owns Gold Teeth, interacting with the item ball reconciles the
local object by setting `0x957` without a duplicate. A full Key Items pocket
leaves the ball and `0x957` unchanged so the player can retry after making
space.
...
checks `ITEM_HM03` ownership. If it is already owned, it records `0x958` only
at this giver and does not duplicate the HM. Otherwise it checks the TM/HM
pocket, attempts the item grant, confirms success, then sets `0x958`. A full
TM/HM pocket leaves `0x958` clear and the attendant available for a later
...
`ITEM_GOLD_TEETH`, set `0x959`, and use the completed dialogue. It also sets
`0x957`, so a previously unclaimed Beach object cannot recreate the returned
Teeth.

If the TM/HM pocket is full, Baoba keeps neither the Teeth nor completion state:
`ITEM_GOLD_TEETH` remains with the player and `0x959` remains clear. The player
...
the relevant Wayfarer runtime tests, and Kanto adventure regressions after
shared generator/runtime changes. Build only in the coordinated map-version
sequence. Report actual test results, release size, and any gaps from this task.
