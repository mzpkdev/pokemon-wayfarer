...
The S.S. Anne port is not implemented. This status describes the existing Aqua
circuit only.

## Future S.S. Anne compatibility

The [S.S. Anne adventure specification](frlg-kanto-ss-anne-adventure.md)
owns its additive top-level Vermilion choice, Ticket check, and audit changes.
That choice must not bypass this feature's regular-Aqua gate, reindex its
six-entry destination submenu, or change Aqua state, routes, initialization,
or standalone HNS behavior. Until the Anne port is implemented, the current
runtime and audits remain the implemented baseline.
...
[future Anne port](#future-ss-anne-compatibility) adds a top-level choice
without moving those existing entries. The
