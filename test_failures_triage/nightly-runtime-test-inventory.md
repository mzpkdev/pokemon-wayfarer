...
The current local evidence used the historical Emerald variant, with `-j22`, and compiled before starting Hydra. Its bounded output in `game/build/triage-evidence/full-strict-final.log` has 3,898 result lines: 1,945 `PASS`, 1,416 `FAIL`, 49 `ASSUMPTION_FAIL`, 8 `KNOWN_FAILING`, 5 `EXPECTED_FAIL`, 474 `TO_DO`, and 1 `CRASH`; no `INVALID` result was observed. The run did not complete and no final status, aggregate, or trustworthy elapsed time was captured, so these are not complete-suite totals.
...
- The current full-run log is 4,676 lines and 378,565 bytes. It is bounded partial evidence only: the run did not complete, and no final status or exact elapsed time was captured.
