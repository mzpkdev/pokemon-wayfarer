...
Reconstructing that ledger found 99 failed result records with a retained case-only pair, correcting an earlier count of 78. Iterating through newly exposed assertions changed 144 exact expectations across 53 test files, including generic stat labels already rendered by the game. Three complete reruns moved another 71 tests from `FAIL` to `PASS` with no reverse transitions. That aggregate was 2,803 `PASS`, 1,653 `FAIL`, 46 `ASSUMPTION_FAIL`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`. Pending-message context fell to 1,647 records.

The ledger's two remaining retained case-only pairs pass after focused alignment. A presentation-only pilot also aligns three Galar form scenarios and 11 directly observed Red Card messages. All three Galar targets pass. Nine Red Card scenarios pass, while two advance to later independent message assertions. That milestone aggregate was 2,817 `PASS`, 1,639 `FAIL`, 46 `ASSUMPTION_FAIL`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`.

Wave 24 recorded 5,149 results: 3,484 `PASS`, 972 `FAIL`, 46 `ASSUMPTIONS_FAILED`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`. Its first residual scan incorrectly considered only the first unmatched actual message. A complete retained-history pass found 242 more directly evidenced expectations. Two stricter follow-up passes found another 55 and five expectations by requiring deterministic unmatched-message failures, direct source literals, no prior or overwritten actuals, and a case-only match to the first retained actual on the same queue.

The resulting Wave 27 ledger recorded the same 5,149 results: 3,671 `PASS`, 785 `FAIL`, 46 `ASSUMPTIONS_FAILED`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`. Across Waves 25 through 27, exactly 302 message expectations changed across 127 test files, with no production or matcher changes. The final paired comparisons found no remaining regressions; an unrelated one-wave X-item Friendship regression passed in isolation and recovered in the next full run. Two independent final scans found no remaining candidate under the strict gate. The residual failures include 717 deterministic unmatched-message results plus probability wrappers and other assertions. These current results do not automatically reclassify rows in this historical inventory.

Wave 28 recorded 5,107 results: 3,677 `PASS`, 783 `FAIL`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`, with no assumption failures. It replaces four Gen 5-only sleep assumptions: three duration-sensitive tests now use exact configured-generation bounds, while the status-cure test keeps its behavioral coverage without an unrelated generation prerequisite. It also aligns 17 directly exercised uppercase message expectations in five test files. Two randomized failures and the four unique sleep tests moved to `PASS`, with no regression. Hydra had repeated the Dark Void and Hypnosis assumptions once per worker, 22 times each. Replacing those 44 duplicate rows with two passing results accounts for the 42-result reduction; all 5,099 normalized identities remain present. A strict residual scan found no further safe message edit because none of the 717 deterministic unmatched-message results meets the full direct-evidence gate. These current results do not automatically reclassify rows in this historical inventory.

Wave 29 aligns 26 exact message expectations across ten test files, including presentation failures hidden behind randomized result wrappers. Nine randomized tests now reach their unchanged declared rates. Three Early Bird tests pass with canonical messages, and the preset sleep-status test covers every timer value allowed by the configured generation. The full ledger retains 5,107 results and records 3,690 `PASS`, 770 `FAIL`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`. Normalized comparison found exactly 13 `FAIL` to `PASS` transitions with no regression, disappearance, or multiplicity change. No RNG rate, configuration, production code, or gameplay changed; the only fixture change is expanded test-only sleep-timer coverage. These current results do not automatically reclassify rows in this historical inventory.
...
- The completed PR #7 CI run is retained as comparable CI evidence. A later attempt did not acquire a hosted runner, but subsequent pushes are acquiring runners normally. The completed comparable run executed all 5,113 tests, exited 2 after reporting failures, and did not time out.
...
- Exact message diagnostics preserve queue matching and result behavior. HnS commit `73c788a6b1` capitalized canonical data names without updating many older message expectations. The evidence-backed Wave 25 through Wave 27 batches changed 302 exact expectations in 127 test files and moved the aggregate from 3,484 to 3,671 passes. Wave 28 adds 17 exact expectations in five test files, and Wave 29 adds 26 across ten test files, raising the aggregate to 3,690 passes. The deterministic residual scan found no further presentation-only candidate under the complete direct-evidence gate. Wave 29 also checked randomized wrappers and retained only focused corrections that reached their unchanged declared rates. Keep exact matching and do not revert production casing because that would change visible gameplay. Accept automatic line wrapping only where decoded expected and observed messages otherwise match exactly.
...
- Hypnosis, Dark Void, and natural-wake Sleep Clause coverage now use the exact range selected by `B_SLEEP_TURNS` instead of assuming Gen 5 or later. The status-cure Sleep Clause test also runs without that unrelated prerequisite. The Gen 3 configuration exercises one through four natural sleep turns, and focused plus full-suite runs pass. This changes test coverage only; production sleep logic and configuration remain untouched.
- The preset sleep-status test now covers every timer value allowed by `B_SLEEP_TURNS`, and all three Early Bird tests use canonical rendered messages. Focused checks and Wave 29 pass. The preset Early Bird timers and Rest behavior remain unchanged.
