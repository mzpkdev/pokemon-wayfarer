...
- Exact message diagnostics preserve queue matching and result behavior. HnS commit `73c788a6b1` capitalized canonical data names without updating many older message expectations. Keep exact matching and update only expectations backed by decoded expected/observed evidence. Do not revert production casing because that would change visible gameplay.
