...
The latest complete local Wave 24 ledger recorded 5,149 results: 3,484 `PASS`, 972 `FAIL`, 46 `ASSUMPTION_FAIL`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`. The test command remained nonzero because ordinary failures remain. A final glyph-aware scan found no presentation-only candidate. The remaining 860 unmatched-message failures split into 726 substantive content, actor, or ordering mismatches, 132 cases with no retained unmatched message for the pending expectation, and 2 AI concatenated-literal cases without a presentation-only counterpart. The other 112 are non-message assertions outside the presentation-only lane. These current results do not automatically reclassify rows in this historical inventory.
