...
- Exact message diagnostics preserve queue matching and result behavior. HnS commit `73c788a6b1` capitalized canonical data names without updating many older message expectations. The evidence-backed Wave 25 through Wave 27 batches changed 302 exact expectations in 127 test files and moved the aggregate from 3,484 to 3,671 passes. The final strict scan found no remaining presentation-only candidate. Keep exact matching and do not revert production casing because that would change visible gameplay. Accept automatic line wrapping only where decoded expected and observed messages otherwise match exactly.
