...
The ledger's two remaining retained case-only pairs pass after focused alignment. A presentation-only pilot also aligns three Galar form scenarios and 11 directly observed Red Card messages. All three Galar targets pass. Nine Red Card scenarios pass, while two advance to later independent message assertions. The latest complete aggregate is 2,817 `PASS`, 1,639 `FAIL`, 46 `ASSUMPTION_FAIL`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`. These results do not automatically reclassify rows in this historical inventory.
...
- Exact message diagnostics preserve queue matching and result behavior. HnS commit `73c788a6b1` capitalized canonical data names without updating many older message expectations. Evidence-backed presentation updates changed 254 tests from `FAIL` to `PASS` in complete comparisons without a status regression. Keep exact matching and do not revert production casing because that would change visible gameplay. Accept automatic line wrapping only where decoded expected and observed messages otherwise match exactly.
