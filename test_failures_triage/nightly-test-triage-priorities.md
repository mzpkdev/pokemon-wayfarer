...
The ledger's final two retained case-only expectations pass after focused alignment. A separate presentation-only pilot aligns three Galar form scenarios and 11 directly observed Red Card messages. All three Galar targets pass. Nine Red Card scenarios pass, while the rooted and Suction Cups scenarios advance to their next independent message assertions. The latest complete run reports 2,817 `PASS`, 1,639 `FAIL`, 46 `ASSUMPTION_FAIL`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`. Other records must not be bulk-classified as capitalization or wrapping failures.
...
8. **Keep the exact presentation expectation batches.** Canonical names were capitalized in HnS commit `73c788a6b1`, while many imported battle expectations still used title case. Evidence-backed batches align expectations with the rendered species, form, move, ability, item, and generic stat labels. The Red Card pilot also accepts automatic line wrapping only where decoded expected and observed messages otherwise match exactly. Complete local comparisons changed 254 tests from `FAIL` to `PASS` and introduced no status regressions. Exact matching remains in place. Do not make matching case-insensitive or revert production names because those options would weaken assertions or change visible gameplay.
...
1. Remaining message classification. The evidence-backed presentation updates reduced the complete local failure count by 254. Missing, extra, and reordered messages remain unresolved. Keep exact matching and do not infer a capitalization or wrapping cause without a directly decoded expected and observed pair.
