...
1. Remaining message classification. The strict presentation-only lane remains exhausted after Wave 28. None of the 717 deterministic unmatched-message results meets the full direct-evidence gate: a decoded case-only or wrapping-only pair on the same queue, with no prior or overwritten actual messages. The remaining message failures involve substantive content, actors, ordering, probability wrappers, overwritten histories, or missing evidence. Keep exact matching and do not infer a capitalization or wrapping cause without a directly decoded expected and observed pair.
