...
Reconstructing that ledger identified 99 failed result records with a retained case-only pair, correcting an earlier count of 78. Iterating through newly exposed sequential assertions updates 144 exact expectations across 53 test files, including generic `ATTACK`, `DEFENSE`, and `SPEED` labels already rendered by the game. Three complete reruns moved another 71 tests from `FAIL` to `PASS`, with no reverse transitions. The latest complete run reported 2,803 `PASS`, 1,653 `FAIL`, and unchanged counts for every other status. Pending-message context fell to 1,647 records. Some edited tests now reach later unrelated assertions, so diagnostic-record and source-assertion counts do not imply the same number of terminal passes. That ledger retained two final case-only pairs. Exact focused reruns pass after aligning both expectations, so no retained case-only pair from the latest complete ledger remains open. Other records must not be bulk-classified as capitalization failures.
...
8. **Keep the exact casing expectation batches.** Canonical names were capitalized in HnS commit `73c788a6b1`, while many imported battle expectations still used title case. Two evidence-backed batches align expectations with the rendered species, move, ability, item, and generic stat labels. Complete local comparisons changed 240 tests from `FAIL` to `PASS` and introduced no status regressions. The last two retained case-only tests also pass focused checks after their expectations were aligned. Exact matching remains in place. Do not make matching case-insensitive or revert production names because those options would weaken assertions or change visible gameplay.
...
1. Remaining message classification. The two exact casing batches reduced the complete local failure count by 240, and the last two retained case-only tests pass focused checks. Missing, extra, and reordered messages remain unresolved. Keep exact matching and do not infer a capitalization cause for records without a case-only pair.
