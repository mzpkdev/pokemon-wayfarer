...
The completed [PR #7 CI run](https://github.com/mzpkdev/pokemon-wayfarer/actions/runs/32980328525) against `5993e0982b` is retained as comparable CI evidence. It reported 5,113 total tests: 2,556 `PASS`, 1,884 `FAIL`, 28 `ASSUMPTION_FAIL`, 11 `KNOWN_FAILING`, 628 `TO_DO`, and 6 `EXPECT_FAILING`. The process exited 2 because tests failed. A later attempt did not acquire a hosted runner, but subsequent PR pushes are acquiring runners normally. That cancellation is not an active task blocker.
...
Of the 1,881 result records with pending-message context, 1,762 captured at least one observed message and 119 captured none. The four-entry bounded history overwrote older observations in 1,344 records, so absence from retained history is not proof that a message never appeared. Exact charmap decoding found 268 records where an expected and observed pair differed only in canonical species, move, ability, or item capitalization. Updating the corresponding exact expectations changed 382 assertions across 113 test files. A complete local rerun moved 169 named tests from `FAIL` to `PASS`, with no `PASS` to `FAIL` transitions. It reported 2,732 `PASS` and 1,724 `FAIL`, and pending-message context fell from 1,881 to 1,716 records.

Reconstructing that ledger identified 99 failed result records with a retained case-only pair, correcting an earlier count of 78. Iterating through newly exposed sequential assertions updates 144 exact expectations across 53 test files, including generic `ATTACK`, `DEFENSE`, and `SPEED` labels already rendered by the game. Three complete reruns moved another 71 tests from `FAIL` to `PASS`, with no reverse transitions. That run reported 2,803 `PASS`, 1,653 `FAIL`, and unchanged counts for every other status. Pending-message context fell to 1,647 records. Some edited tests now reach later unrelated assertions, so diagnostic-record and source-assertion counts do not imply the same number of terminal passes.

The ledger's final two retained case-only expectations pass after focused alignment. A separate presentation-only pilot aligns three Galar form scenarios and 11 directly observed Red Card messages. All three Galar targets pass. Nine Red Card scenarios pass, while the rooted and Suction Cups scenarios advance to their next independent message assertions. That milestone reported 2,817 `PASS`, 1,639 `FAIL`, 46 `ASSUMPTION_FAIL`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`.

Wave 24 recorded 5,149 results: 3,484 `PASS`, 972 `FAIL`, 46 `ASSUMPTIONS_FAILED`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`. The first residual scan incorrectly considered only the first unmatched actual message. A complete retained-history scan found 242 more directly evidenced expectations. Wave 25 improved to 3,621 `PASS` and 835 `FAIL`; 138 edited records passed and 102 advanced to later assertions. One unrelated X-item Friendship result briefly regressed, passed in isolation, and recovered in Wave 26.

The next two batches used a stricter gate: deterministic top-level unmatched-message failures only, direct one-line literals, no prior or overwritten actual messages, and a case-only match to the first retained actual on the same queue. Two independent generators agreed on 55 rows for Wave 26 and five rows for Wave 27. Wave 26 recorded 3,666 `PASS` and 790 `FAIL`; Wave 27 recorded 3,671 `PASS` and 785 `FAIL`. Neither comparison had a regression. Across the three batches, exactly 302 message expectations changed in 127 test files, with no production or matcher changes. Two independent final scans found no remaining strict presentation-only candidate. The remaining 785 failures include 717 deterministic unmatched-message results plus probability wrappers and other assertions. None authorizes a gameplay, configuration-default, save-compatibility, or UI-behavior change in this task.

Wave 28 removes four Gen 5-only sleep assumptions. Hypnosis and Dark Void now check the configured generation's exact sleep range, and the natural-wake Sleep Clause test waits through that range: three turns for Gen 5 and later, four for Gen 3 and 4, and seven for older generations. The status-cure Sleep Clause test keeps its behavioral coverage without the unrelated generation prerequisite. Wave 28 also aligns 17 exact message expectations across five test files, including two randomized tests that were outside the earlier deterministic scan. The full run recorded 5,107 results: 3,677 `PASS`, 783 `FAIL`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`, with no assumption failures or regressions. The apparent 42-result reduction is Hydra multiplicity, not missing coverage: 44 worker-duplicated Dark Void and Hypnosis assumption rows became two ordinary passing results, while the other two sleep assumptions each became a pass. All 5,099 normalized test identities remain present.

Wave 29 checks randomized wrappers whose successful trials were still blocked by title-case presentation literals. It aligns 26 exact message expectations across ten test files. Nine randomized tests now pass at their unchanged declared rates, all three Early Bird tests pass, and the preset sleep-status test covers every timer value allowed by the configured generation. The full run retained 5,107 results and recorded 3,690 `PASS`, 770 `FAIL`, 12 `KNOWN_FAILING`, 629 `TO_DO`, and 6 `EXPECT_FAILING`. Normalized comparison found exactly 13 `FAIL` to `PASS` transitions and no regression, disappearance, or multiplicity change. No RNG rate, configuration, production code, or gameplay changed; the only fixture change is expanded test-only sleep-timer coverage.
...
8. **Keep the exact presentation expectation batches.** Canonical names were capitalized in HnS commit `73c788a6b1`, while many imported battle expectations still used title case. Evidence-backed batches align expectations with the rendered species, form, move, ability, item, generic stat labels, glyphs, and automatic wrapping only where decoded expected and observed content otherwise matches exactly. Waves 25 through 27 added 302 exact substitutions across 127 test files, moving the full suite from 3,484 to 3,671 passes. Wave 28 adds 17 exact expectations in five test files and moves two randomized failures plus four newly exercised sleep tests to `PASS`. Wave 29 adds 26 expectations in ten test files and moves another 13 tests to `PASS`, including nine randomized wrappers. Exact matching remains in place. Do not make matching case-insensitive or revert production names because those options would weaken assertions or change visible gameplay.
...
1. Remaining message classification. The deterministic top-level presentation lane remains exhausted after Wave 29. None of its unmatched-message results meets the full direct-evidence gate: a decoded case-only or wrapping-only pair on the same queue, with no prior or overwritten actual messages. Wave 29 separately checked randomized wrappers against source-local canonical messages and retained only focused cases that reached their unchanged declared rates. A final wrapper scan found no remaining candidate. The residual message failures involve substantive content, actors, ordering, probability behavior, overwritten histories, or missing evidence. Keep exact matching and do not infer a capitalization or wrapping cause without direct renderer or focused evidence.
