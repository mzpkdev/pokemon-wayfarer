...
8. **Keep the canonical-name expectation batch.** Canonical names were capitalized in HnS commit `73c788a6b1`, while many imported battle expectations still used title case. The evidence-backed batch updates expectations to current rendered species, move, ability, and item names. The full local comparison changed 169 named tests from `FAIL` to `PASS` and introduced no status regressions. Exact matching remains in place. Do not make matching case-insensitive, normalize generic labels, or revert production names because those options would weaken assertions or change visible gameplay.
...
1. Remaining message classification. The first canonical-name batch is complete and reduced the full local failure count by 169. The new ledger has 78 failed records with a retained case-only pair, mostly involving generic display labels, plus unresolved missing, extra, or reordered messages. Keep exact matching and do not infer the capitalization cause for records without a case-only pair.
