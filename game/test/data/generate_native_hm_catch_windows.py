...
baseline = json.loads((REVISION.parents[1] / 'baseline.json').read_text())['species']
delta_path = helper_path.with_name('native_hm_upstream_delta.json')
lines = [f'// Approved roster SHA256: {hashlib.sha256(roster_path.read_bytes()).hexdigest()}',
         f'// Pinned upstream move delta SHA256: {hashlib.sha256(delta_path.read_bytes()).hexdigest()}',
...
for species in roster:
    for modern, mode in enumerate(('legacy', 'modern')):
        ...
        for level in range(1, 101):
            moves = []
            for entry_level, move in entries:
