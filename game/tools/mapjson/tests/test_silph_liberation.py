...
class SilphLiberationMapTest(unittest.TestCase):
    ...
    def test_steven_keeps_the_existing_lobby_gift_independent_of_silph(self):
        lobby = json.loads(
            (GAME_ROOT / "data/maps/SaffronCity_SilphCo_hns/map.json").read_text()
        )
        steven = next(event for event in lobby["object_events"]
                      if event.get("local_id") == "LOCALID_SILPHCO_STEVEN")
        self.assertEqual(steven["script"], "SaffronCity_SilphCo_EventScript_Steven")
        self.assertEqual(steven["flag"], "FLAG_GOT_HOENN_STARTER")

        scripts = (GAME_ROOT / "data/maps/SaffronCity_SilphCo_hns/scripts.inc").read_text()
        gift = scripts.split("SaffronCity_SilphCo_EventScript_Steven::", 1)[1].split(
            "SaffronCity_SilphCo_EventScript_Receptionist::", 1)[0]
        self.assertNotIn("#if IS_WAYFARER", gift)
        self.assertNotIn("FLAG_SILPH_LIBERATED_HNS", gift)
        self.assertIn("MULTI_HOENN_STARTERS", gift)
        for species in ("TREECKO", "TORCHIC", "MUDKIP"):
            self.assertIn(f"givemon SPECIES_{species}, 5", gift)
        self.assertIn("setflag FLAG_GOT_HOENN_STARTER", gift)
