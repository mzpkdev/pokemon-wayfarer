...
class FuchsiaSafariWayfarerTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        # Map event generation also reads the generated map-groups header for
        # connection names. Keep its small fixture outside the source tree.
        cls.fixture_root = Path(cls.build_dir.name) / "fixture"
        (cls.fixture_root / "include/constants").mkdir(parents=True)
        (cls.fixture_root / "data").symlink_to(GAME_ROOT / "data")
        (cls.fixture_root / "include/constants/map_groups.h").write_text(
            "MAP_FUCHSIA_CITY_SAFARI_ZONE_BRUSH_HNS = (0\n"
            "MAP_FUCHSIA_CITY_SAFARI_ZONE_MOUNTAIN_HNS = (0\n"
        )
    ...
    def test_flag_namespace_and_transaction_boundaries(self):
        ...
        self.assertEqual(
            ...
            {
                "FLAG_FUCHSIA_SAFARI_GOLD_TEETH_CLAIMED_HNS": 0x957,
                "FLAG_FUCHSIA_SAFARI_SURF_RECEIVED_HNS": 0x958,
                "FLAG_FUCHSIA_SAFARI_STRENGTH_RECEIVED_HNS": 0x959,
            },
        )
...
_Static_assert(FLAG_FUCHSIA_SAFARI_GOLD_TEETH_CLAIMED_HNS == 0x957, \"Gold Teeth flag\");
_Static_assert(FLAG_FUCHSIA_SAFARI_SURF_RECEIVED_HNS == 0x958, \"Surf flag\");
_Static_assert(FLAG_FUCHSIA_SAFARI_STRENGTH_RECEIVED_HNS == 0x959, \"Strength flag\");
...
"""
        result = subprocess.run(
            ["gcc", "-std=c11", "-DIS_HNS=1", "-DIS_WAYFARER=1", "-Iinclude", "-x", "c", "-fsyntax-only", "-"],
            cwd=GAME_ROOT,
            text=True,
            input=assertion,
            capture_output=True,
