...
class MtMoonWayfarerTest(unittest.TestCase):
    ...
    def test_connected_story_routes(self):
        # The seven story objects leave a connected path between every cave
        # entrance, each interaction approach, and Silver.  The sign and
        # hidden item sit on blocked tiles, so their adjacent walkable tiles
        # are used here.
        layout = self.layouts["LAYOUT_MT_MOON_CAVE_HNS"]
        occupied = {
            (event["x"], event["y"])
            for event in self.map_data["object_events"] if event.get("wayfarer_only")
        }
        start = (10, 15)
        reached = {start}
        pending = deque([start])
        while pending:
            ...
            for adjacent in ((x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)):
                ...
                if not (0 <= nx < layout["width"] and 0 <= ny < layout["height"]):
                    continue
                if adjacent in reached or adjacent in occupied or self.grid(nx, ny)[1] != 0:
                    continue
                reached.add(adjacent)
                pending.append(adjacent)
        self.assertEqual(len(reached), 682)
        for approach in ((10, 15), (38, 16), (44, 28), (33, 16),
                         (25, 16), (24, 16), (26, 16), (9, 12),
                         (16, 14), (27, 21), (46, 31), (4, 12),
                         (24, 10), (32, 21)):
            self.assertIn(approach, reached)

    def test_local_state_and_dialogue_are_wayfarer_scoped(self):
        macros = {}
        for define, wayfarer in (("POKEMON_WAYFARER", 1), ("POKEMON_HNS", 0)):
            output = subprocess.run(
                ["cpp", "-dM", f"-D{define}", f"-DIS_WAYFARER={wayfarer}",
        ...
        expected = {
            "FLAG_HIDE_MT_MOON_DOME_FOSSIL_HNS": "0x954",
            "FLAG_HIDE_MT_MOON_HELIX_FOSSIL_HNS": "0x955",
            "FLAG_GOT_FOSSIL_FROM_MT_MOON_HNS": "0x956",
        }
