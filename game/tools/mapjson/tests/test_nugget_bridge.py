...
class NuggetBridgeTest(unittest.TestCase):
    ...
    def test_original_route_and_neighbor_stories_are_unchanged(self):
        ...
        files = {
            'data/maps/Route24_hns/scripts.inc': '774a527e23c07627ce021f1e14ffbf2a914d1b259dde86bb1e49199223506318',
    ...
    def test_generator_preserves_hns_and_assigns_appended_local_ids(self):
        # Map generation reads the generated map catalog from the working
        # directory. Supply the three connection maps in an isolated fixture.
        (self.root / 'include/constants').mkdir(parents=True)
        (self.root / 'include/constants/map_groups.h').write_text(
            'MAP_ROUTE24_HNS = (1)\nMAP_CERULEAN_CITY_HNS = (2)\nMAP_ROUTE25_HNS = (3)\n')
        (self.root / 'data').symlink_to(GAME_ROOT / 'data')
        generated = {}
        for version in ['hns', 'wayfarer']:
            ...
            subprocess.run([str(self.tool), 'map', version, 'data/maps/Route24_hns/map.json',
                            'data/layouts/layouts.json', str(output)], cwd=self.root, check=True, capture_output=True)
            generated[version] = (output / 'events.inc').read_text()
            ids = output / 'ids.h'
            subprocess.run([str(self.tool), 'event_constants', version,
                            'data/maps/Route24_hns/map.json', str(ids)], cwd=self.root, check=True)
    ...
    def test_receipt_is_wayfarer_local(self):
        for wayfarer, expected in [(0, '0'), (1, '0x95A')]:
            output = subprocess.run(
                ['cpp', '-dM', f'-DIS_WAYFARER={wayfarer}', '-I', str(GAME_ROOT / 'include'),
