...
class SilphContracts(unittest.TestCase):
    def test_authored_rosters_ids_and_policy(self):
        authored = (gen.ROOT / 'src/data/trainers_wayfarer_local.party').read_text()
    ...
    def test_transaction_and_loss_boundaries(self):
        ...
        self.assertLess(president.index('checkitemspace ITEM_MASTER_BALL'), president.index('additem ITEM_MASTER_BALL'))
        self.assertLess(president.index('additem ITEM_MASTER_BALL'), president.index('goto_if_eq VAR_RESULT, FALSE, WayfarerSilph_EventScript_PresidentNoRoomForMasterBall', president.index('additem ITEM_MASTER_BALL')))
        self.assertLess(president.index('msgreceiveditem SilphCo_11F_Text_ObtainedMasterBallFromPresident'), president.index('setflag FLAG_SILPH_MASTER_BALL_RECEIVED_HNS'))
