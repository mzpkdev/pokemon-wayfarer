...
class InventoryTests(unittest.TestCase):
    def test_real_compiler_parses_custom_move_tuple(self):
        ...
        custom = [slot for row in records.values() for slot in row['slots'] if 'moves' in slot]
    ...
    def test_real_compiler_uses_wayfarer_roster_macros(self):
        records = gen.load_inventory()
        self.assertEqual(records['TRAINER_PHOEBE']['slots'][0]['lvl'], 87)

    def test_alias_chain_and_pool_keep_original_slots(self):
        records = {'TRAINER_A': roster([], overrideTrainer='TRAINER_B'), 'TRAINER_B': roster([], overrideTrainer='TRAINER_C'), 'TRAINER_C': roster([{'species': 'SPECIES_RATTATA', 'lvl': 10}, {'species': 'SPECIES_PIDGEY', 'lvl': 20}], poolSize=2)}
        ...
        self.assertEqual(records['TRAINER_A']['slots'], [])
    ...
    def test_alias_cycle_and_unknown_owner_fail(self):
        for target in ('TRAINER_A', 'TRAINER_UNKNOWN'):
            with self.assertRaises(gen.ValidationError):
                gen.resolve_rosters({'TRAINER_A': roster([], overrideTrainer=target)})

    def test_one_authored_record_is_preserved(self):
        records = {'TRAINER_A': roster([{'species': 'SPECIES_RATICATE', 'lvl': 40}])}
        resolved = gen.resolve_rosters(records)['TRAINER_A']
        self.assertEqual(resolved['slots'][0]['lvl'], 40)

    def test_missing_duplicate_and_unknown_manifest_ids_fail(self):
        records = {'TRAINER_A': roster()}
    ...
    def test_hoenn_record_is_direct(self):
        records = {'TRAINER_A': roster(source='src/data/trainers.party')}
        resolved = gen.resolve_rosters(records)['TRAINER_A']
        self.assertEqual(resolved['owner'], 'TRAINER_A')
    ...
    def test_proposal_does_not_match_boss_name_substrings(self):
        records = {'TRAINER_ALFRED_HNS': roster(source='src/data/trainers_hns.party')}
