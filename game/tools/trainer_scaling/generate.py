...
ANNE_ALLOCATION = ROOT.parent / 'docs/ss-anne-trainer-implementation/allocation-inventory.json'
ANNE_ROSTER = ROOT / 'src/data/trainers_wayfarer_ss_anne.h'
TOWER_ROSTER = ROOT / 'src/data/trainers_wayfarer_tower.h'
INDIGO_ROSTER = ROOT / 'src/data/trainers_wayfarer_indigo.h'
...
def load_inventory():
    ...
    result.update(coast)
    if not ANNE_ROSTER.is_file():
        raise ValidationError(f'missing generated S.S. Anne roster: {ANNE_ROSTER}')
    anne = parse_output(ANNE_ROSTER.read_text(), 'src/data/trainers_wayfarer_ss_anne.h')
    overlap = result.keys() & anne.keys()
    if overlap: raise ValidationError(f'duplicate selected S.S. Anne IDs: {sorted(overlap)}')
    result.update(anne)
    if not TOWER_ROSTER.is_file():
        raise ValidationError(f'missing selected Pokemon Tower roster: {TOWER_ROSTER}')
    tower = parse_output(TOWER_ROSTER.read_text(), 'src/data/trainers_wayfarer_tower.h')
    overlap = result.keys() & tower.keys()
    if overlap: raise ValidationError(f'duplicate selected Pokemon Tower IDs: {sorted(overlap)}')
    result.update(tower)
    if not CELADON_HIDEOUT_ROSTER.is_file():
        raise ValidationError(f'missing generated Celadon Hideout roster: {CELADON_HIDEOUT_ROSTER}')
    hideout = parse_output(CELADON_HIDEOUT_ROSTER.read_text(), 'src/data/trainers_wayfarer_celadon_hideout.h')
    overlap = result.keys() & hideout.keys()
    if overlap: raise ValidationError(f'duplicate Celadon Hideout IDs: {sorted(overlap)}')
    result.update(hideout)
    if not INDIGO_ROSTER.is_file():
        raise ValidationError(f'missing selected Indigo roster: {INDIGO_ROSTER}')
    indigo = parse_output(INDIGO_ROSTER.read_text(), 'src/data/trainers_wayfarer_indigo.h')
    if len(indigo) != 5 or any(not ident.startswith('TRAINER_WAYFARER_INDIGO_') for ident in indigo):
        raise ValidationError('Indigo selected roster must contain exactly five Trainers')
    overlap = result.keys() & indigo.keys()
    if overlap: raise ValidationError(f'duplicate Indigo IDs: {sorted(overlap)}')
    result.update(indigo)
    return result

def load_indigo_manifest():
    indigo = parse_output(INDIGO_ROSTER.read_text(), 'src/data/trainers_wayfarer_indigo.h')
    return [{'id': ident, 'policy': 'EXCLUDED', 'region': 'Kanto',
             'reason': 'Indigo circuit uses its own saved run rating and League scaling',
             'evidence': [{'path': 'src/data/trainers_wayfarer_indigo.h', 'symbol': ident}]}
            for ident in indigo]

def load_celadon_hideout_manifest():
    ids = [*(f'TRAINER_CELADON_HIDEOUT_GRUNT_{i}_HNS' for i in range(7, 19)),
           'TRAINER_CELADON_HIDEOUT_GIOVANNI_HNS']
    result = []
    for ident in ids:
        boss = ident.endswith('GIOVANNI_HNS')
        row = {'id': ident, 'policy': 'EXCLUDED' if boss else 'ORDINARY', 'region': 'Kanto',
               'evidence': [{'path': 'src/data/trainers_wayfarer_celadon_hideout.h', 'symbol': ident}]}
        if boss:
            row['reason'] = 'Authored Celadon Hideout boss encounter'
        result.append(row)
    return result

def load_tower_manifest():
    """Classify the selected ordinary Channelers and Rockets for rating scaling."""
    tower = parse_output(TOWER_ROSTER.read_text(), 'src/data/trainers_wayfarer_tower.h')
    if len(tower) != 16 or any(not ident.startswith('TRAINER_WAYFARER_TOWER_') for ident in tower):
        raise ValidationError('Pokemon Tower selected roster must contain 16 ordinary Trainers')
    return [{'id': ident, 'policy': 'ORDINARY', 'region': 'Kanto',
             'evidence': [{'path': 'src/data/trainers_wayfarer_tower.h', 'symbol': ident}]}
            for ident in tower]

def load_anne_manifest():
    try:
        allocation = json.loads(ANNE_ALLOCATION.read_text())['allocation']['allocations']
    except (OSError, KeyError, TypeError, json.JSONDecodeError) as error:
        raise ValidationError(f'cannot load S.S. Anne scaling inventory: {error}')
    if len(allocation) != 16 or [row.get('slot') for row in allocation] != list(range(16)):
        raise ValidationError('S.S. Anne allocation is not the frozen dense 16-slot inventory')
    return [{'id': row['id'], 'policy': 'ORDINARY', 'region': 'Kanto',
             'evidence': [{'path': 'src/data/trainers_wayfarer_ss_anne.h', 'symbol': row['id']}]} for row in allocation]
...
def generate(check=False, proposal=None, no_audit=False):
    ...
    manifest = dict(manifest, records=manifest.get('records', []) + load_sevii_manifest() + load_coast_manifest() + load_anne_manifest() + load_celadon_hideout_manifest() + load_tower_manifest() + load_indigo_manifest())
    validate_manifest(manifest, records, ids, refs)
    rows = sorted(manifest['records'], key=lambda row: ids[row['id']])
    header = '// Generated by tools/trainer_scaling/generate.py; edit classification.json or the selected Trainer inventories.\n'
