...
def generate(check=False, proposal=None, no_audit=False):
    ...
    manifest = dict(manifest, records=manifest.get('records', []) + load_sevii_manifest())
    validate_manifest(manifest, records, ids, refs)
    rows = sorted(manifest['records'], key=lambda row: ids[row['id']])
    header = '// Generated by tools/trainer_scaling/generate.py; edit classification.json or sevii_policy.json.\n'
