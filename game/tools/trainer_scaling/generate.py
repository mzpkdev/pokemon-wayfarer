...
def legacy_manifest(records, refs):
    """Return the former local manifest as a derived compatibility projection."""
    if str(ROOT) not in sys.path:
        sys.path.insert(0, str(ROOT))
    from tools.gameplay_content.trainers import (compile_wayfarer_scaling_projection,
                                                 load_legacy_trainers)
    legacy_rows = load_legacy_trainers(ROOT, 'wayfarer', records, partial=True)
    content = compile_wayfarer_scaling_projection(ROOT, records)
    policies = content['scaling']['policies']
    sources = {}
    for encounter in content['encounters']:
        sources.setdefault(encounter['caller']['trainerId'], []).append(encounter['caller']['source'])
    legacy_by_trainer = {row['trainer']: row for row in legacy_rows}
    rows = []
    for trainer, policy in sorted(policies.items()):
        row = legacy_by_trainer.get(trainer)
        # Regional grouping remains an audit projection.  Reviewed source
        # references identify the source input; no region fact is authored in
        # shared.json.
        evidence = ([reference['path'] for reference in row['review']['sourceRefs']]
                    if row else sorted(set(sources.get(trainer, ()))))
        evidence.extend(refs.get(trainer, ()))
        record = dict(id=trainer, policy=policy,
                      region=region_from_evidence(trainer, evidence))
        if row and 'reason' in row['review']:
            record['reason'] = row['review']['reason']
        elif policy == 'EXCLUDED':
            record['reason'] = 'All selected callers use reviewed excluded-policy encounter contracts.'
        rows.append(record)
    return {'version': 1, 'records': rows, 'move_exceptions': []}
...
def generate(check=False, proposal=None, no_audit=False):
    ...
    if proposal:
        ...
        return
    # Phase D owns reviewed fallback policy in the shared gameplay declaration.
    # Keep this local shape only as a generated compatibility view for the
    # established scaling output and audit consumers.
    manifest = legacy_manifest(records, refs)
    rows = sorted(manifest['records'], key=lambda row: ids[row['id']])
    header = '// Generated by tools/trainer_scaling/generate.py; edit data/gameplay/shared.json.\n'
