...
static void DebugAction_Trainers_SetRematch(u8 taskId)
{
    ...
    if (rematchId == -1)
    {
        if (HasTrainerBeenFought(sDebugMenuListData->data[0]))
            ClearTrainerFlag(sDebugMenuListData->data[0]);
        else
            SetTrainerFlag(sDebugMenuListData->data[0]);
        return;
    }
    ...
    for (u32 i = 0; i < REMATCHES_COUNT; i++)
    {
        ...
        {
            SetTrainerFlag(gRematchTable[rematchId].trainerIds[i]);
            return;
        }
    }
    ...
    for (u32 i = 0; i < REMATCHES_COUNT; i++)
    {
        ...
        ClearTrainerFlag(gRematchTable[rematchId].trainerIds[i]);
    }
}
...
{
    ZeroPlayerPartyMons();
    CreateNPCTrainerPartyFromTrainer(gPlayerParty, &sDebugTrainers[DEBUG_TRAINER_PLAYER], TRUE, BATTLE_TYPE_TRAINER);
    ScriptContext_Enable();
    Debug_DestroyMenu_Full(taskId);
}
...
static void DebugAction_Party_BattleSingle(u8 taskId)
{
    ...
    if (sDebugTrainers[DEBUG_TRAINER_AI].battleType == TRAINER_BATTLE_TYPE_DOUBLES)
        gBattleTypeFlags |= BATTLE_TYPE_DOUBLE;
    gDebugAIFlags = sDebugTrainers[DEBUG_TRAINER_AI].aiFlags;
    gIsDebugBattle = TRUE;
    gBattleEnvironment = BattleSetup_GetEnvironmentId();
    CalculateEnemyPartyCount();
    BattleSetup_StartTrainerBattle_Debug();
