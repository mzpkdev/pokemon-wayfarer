...
static void CompleteWhenChoseItem(enum BattlerId battler)
{
    if (gMain.callback2 == BattleMainCB2 && !gPaletteFade.active)
    {
        if (IsOakBattleTutorial()
         && !BtlCtrl_OakOldMan_TestState2Flag(FIRST_BATTLE_MSG_FLAG_HP_RESTORE)
    ...
    else
    {
        ...
        if (gBattleTypeFlags & BATTLE_TYPE_FIRST_BATTLE)
        {
            ...
            case STRINGID_DEFENDERSSTATFELL:
                if (IsOakBattleTutorial() && !BtlCtrl_OakOldMan_TestState2Flag(FIRST_BATTLE_MSG_FLAG_STAT_CHG))
                {
