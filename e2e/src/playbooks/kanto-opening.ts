...
export const receivePalletStarter = async (game: GameSession): Promise<void> => {
  ...
  await game.player.warp("oak-lab", 15, 13, "up")
  await game.player.interact()
  await advanceKantoScene(
    game,
  ...
  await advanceKantoScene(
    game,
    (state) => sceneFinished(state) || isNicknamePrompt(state),
    "Kanto starter nickname prompt",
  )
  if (isNicknamePrompt(await game.state.read())) {
...
const isNicknamePrompt = (state: GameState): boolean =>
  state.choice.kind === "yes-no" && state.dialogue.fullText.includes("nickname")
...
export const resolvePalletBlueBattle = async (
  ...
  // Leave through the left lane so a non-center exit trigger is covered too.
  await game.player.warp("oak-lab", 12, 15, "down")
  await game.player.move("down")
...
export const checkViridianOpeningBoundary = async (game: GameSession): Promise<void> => {
  ...
  if (!gramps.dialogue.fullText.includes("GYM is always closed"))
  ...
  await game.player.warp("viridian-city", 43, 16, "up")
  await game.player.move("up")
  await game.wait.until(
    (state) => state.dialogueOpen && state.dialogue.fullText.includes("doors are locked"),
  ...
  // A sideways step onto the doorstep skips the door trigger; the Gym itself
  // then refuses the opening and returns the player outside.
  await game.player.warp("viridian-city", 42, 15, "right")
  ...
  await game.player.move("up")
  await game.wait.until(
    (state) => state.dialogueOpen && state.dialogue.fullText.includes("closed for now"),
    "Viridian Gym interior refusal",
    1_800,
  )
  await advanceKantoScene(
    game,
    (state) => state.map.name === "viridian-city" && sceneFinished(state),
    "Viridian Gym interior return",
  )
...
/** After the handoff, Viridian's Gym follows the native finale for every origin. */
export const checkViridianAfterOpening = async (game: GameSession): Promise<void> => {
  const blueBefore = await game.story.flag("viridianBlueIntroduced")
  const gymBefore = await game.story.flag("defeatedViridianGym")
  ...
  await game.dialogue.waitForOpen()
  const gramps = await game.state.read()
  if (gramps.dialogue.fullText.includes("always closed"))
    throw new Error(`Viridian Gym gramps kept the opening line: ${gramps.dialogue.fullText}`)
  await advanceKantoScene(game, sceneFinished, "Viridian Gym gramps")

  // The door no longer refuses the Pallet origin once the opening is over.
  await game.player.warp("viridian-city", 43, 16, "up")
  await game.player.move("up")
  await game.wait.frames(20)
  await game.player.move("up")
  await game.wait.until(
    (state) => state.map.name === "viridian-gym" && sceneFinished(state),
    "Viridian Gym entry after the opening",
    1_800,
  )
  ...
  if (
    ...
    throw new Error("Viridian Blue/Gym state changed after the opening")
}
