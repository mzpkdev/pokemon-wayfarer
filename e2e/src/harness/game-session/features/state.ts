...
export type GameState = {
  ...
  dialogue: {
    ...
    rawText: number[]
    fullRawText: number[]
    /** The finished message still waits for A/B before its script continues. */
...
export const decodeFieldMessageText = (bytes: number[]): string => {
  const characters: Record<number, string> = {
    ...
    0x35: "=",
    0x36: ";",
    // Battle strings substitute the non-breaking space; it renders as a space.
    0x39: " ",
    0x51: "¿",
    0x52: "¡",
    0x5a: "Í",
  ...
  read: async () => {
    ...
    return {
      ...
      dialogue: {
        ...
        awaitingButton: snapshot.awaitingButton,
      },
