...
const otherGrunts = [
  { x: 38, y: 16, flag: "mtMoonGrunt2Defeated" },
  { x: 44, y: 28, flag: "mtMoonGrunt3Defeated" },
  { x: 33, y: 16, flag: "mtMoonGrunt4Defeated" },
] as const
...
describe.sequential("Wayfarer Mt. Moon fossils", () => {
  ...
  for (const grunt of otherGrunts) {
    it(`records Grunt ${otherGrunts.indexOf(grunt) + 2} only after a won battle`, async () => {
      await game.arrange({
        checkpoint: "new-bark-after-intro",
        player: { position: { map: "mt-moon-cave", x: grunt.x - 1, y: grunt.y - 1 }, facing: "right" },
        party: [{ species: "lapras", level: 100 }],
        determinism: { textSpeed: "instant" },
      })
      expect(await game.story.flag(grunt.flag)).toBe(false)
      await game.player.interact()
      await game.dialogue.waitForOpen()
      for (let attempt = 0; attempt < 180; attempt++) {
      ...
      expect((await game.state.read()).battle.ui).toBe("action-menu")
      await game.battle.win()
      await settle(game, "mt-moon-cave", `Mt. Moon Grunt ${otherGrunts.indexOf(grunt) + 2} victory`)
      expect(await game.story.flag(grunt.flag)).toBe(true)
      await game.saveAndReload()
      expect(await game.story.flag(grunt.flag)).toBe(true)
      expect(await game.story.flag("receivedMtMoonFossil")).toBe(false)
    })
