...
const waitForForfeitPrompt = async (game: GameSession): Promise<void> => {
  ...
  for (let attempt = 0; attempt < 240; attempt++) {
    ...
    // The current E2E ABI reports the native Yes/No box as "other" and does
    // not expose its cursor. Wait for it after the preceding battle text.
    if (sawPromptText && state.battle.active && state.battle.ui === "other") {
      await game.wait.frames(30)
      if ((await game.state.read()).battle.ui === "other") return
    }
    await game.wait.frames(12)
  }
